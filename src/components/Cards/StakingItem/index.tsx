...
const StakingItem: React.FC<IStakingProps> = ({ item }) => {
  const { bucket, asset, epoch: currentEpoch, rewards } = item;
      ...
      if (asset.assetId === 'KLV') {
        return (
          <Button
            styleType="transparent"
            onClick={() => {
              history.push(`/staking/claim/${bucket.id}`);
            }}
          >
            Claim
          </Button>
  ...
  return bucket ? (
    <Container>
      ...
      <ItemCol>{(rewards / 10 ** asset.precision).toLocaleString()}</ItemCol>
      <ItemCol>{getClaim()}</ItemCol>
